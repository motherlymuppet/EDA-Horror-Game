...
The timing and pace of scares is important to maximise their effect.
...
However, \cite{rosenberg10} warns that jump-scares can be `scatter[ed] liberally thoughout the film to the point where it becomes almost numbing'.
...
Rarely, horror games support the use of heart-rate monitors to tailor gameplay.
Examples include \emph{Nevermind} \citep{nevermind}, and \emph{Bring to Light} \citep{bringtolight}.
In both instances, the games could be played without the sensor.

The use of heart rate monitors in these games is not ideal, but makes sense for a mass-market game as heart rate sensors are some of the most widely available biosignals.
...
The project changed allowed the biofeedback to change a number of independent variables simultaneously, including the level design, asset generation, and event generation.
This meant that the effect of each aspect of the biofeedback was unclear.
This project will only allow the biosignals to control event generation, to gain a clearer picture on its benefits.

There exist a number of frameworks for guiding the design of horror games.
\citet{habel14} introduces the idea of `agency mechanics', suggesting that a horror game should `manage the oscillation between autonomy and control'.
In a horror game that uses predominantly jump-scares, the loss of control is represented by the scare and the autonomy by the periods of calm between scares.
This project aims to tailor the oscillation between autonomy and control on an individual level.

\subsection{Biosignals}
Biosignals are simply any signal in living beings that are time-varying and can be continuously measured.
...
EDA, which goes under many other names including the `Galvanic Skin Response' (GSR) and `Skin Conductance Response' (SCR), measures how the electrical resistance of the skin changes.
...
As such, we will use \citet{boucsein12} as our only resource on EDA as it provides an incredibly in-depth and easily understandable look at the mechanics of EDA.
It is probably the only resource on EDA you will ever need.

EDA measures the electrical resistance of the skin.
When the body becomes more alert, the sympathetic nervous system causes slight sweating all of the body.
...
EDA is used in many applications, including as part of the polygraph (lie detector) test, where it can detect the alertness caused by lying and being afraid of the repercussions of doing so \citep{polygraph}.
...
That high base value is rarely reached because arousal varies frequently even without any explicit stimulus, so the EDA response happens quite often.

A visual example of data collected from a user who is playing a horror game is provided in \vref{fig:ExampleEda}.
