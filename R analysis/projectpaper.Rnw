...
%This study finds that participants are more scared on average when the timing of the scares is tailored based on biometric data than when scares are triggered randomly.
...
\begin{keywords}
Biometrics, Electro-dermal Activity, Human-Computer Interaction, Video Games, Horror
\end{keywords}
...
As outlined above, the fundamental research question is: \emph{Can biometric sensors be used to tailor the timing of scares in a horror game to improve the user experience?}. To answer this question, the following research objectives must be completed:
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Create one or more algorithms using biometric data which trigger events at moments of maximum impact.
		\item Create a (null hypothesis) algorithm for random generation of events.
	\end{enumerate}
...
Addressing these objectives, this paper discusses how scares were tailored based on biometric data and the design of the experimental study.
...
%TODO mini conclusion. Refer to the results.
...
This section will discuss the previous research and studies that are relevant to the project.
There is little academic research on tailoring horror games to individuals, and none that uses EDA to do so.
Therefore, this section will begin by examining the few academic sources available, before moving to non-academic sources which more closely relate to the study.
...
When the body is becoming more aroused, it leads to an increase in electro-dermal activity (EDA).
...
GSR (galvanic skin response) is a synonymous (though outdated) term for EDA, which is notably used in the name of the EDA sensor that the user study uses. %TODO SHIT
...
An EDA response consists of a sharp drop in resistance, over a period of {\raise.17ex\hbox{$\scriptstyle\sim$}}1s, when arousal passes a certain threshold (the size of the drop correlates with the extent of the arousal), followed by an asymptotic increase in resistance back to a high base value, with a half-life of {\raise.17ex\hbox{$\scriptstyle\sim$}}10s.
...
Attempting infer a stimulus from a reaction, as in the initial work in \vref{sec:Initial}, is difficult and error-prone.
...
%TODO explain how biometrics are used - we use to measure EDA

Biometrics are%TODO define, cite, examples of sensors, examples of use
...
%TODO horror game design

%Focus on the prior work by other people in the horror games bit instead of horror games in general - the bit about other attempts at tailoring are the most interesting

The horror games industry is huge, with over 2000 unique titles on the digital games platform \emph{Steam} and almost 500 million copies owned \citep{horrorSteamSpy}.
...
The effectiveness of heart rate sensors is limited by slow response times, leading to an inability to show the immediate response caused by a scare.
...
The horror genre is an ideal first candidate for integrating biometrics into games.
Since the game triggers scares, we have a defined event which we expect to cause a large reaction, and can use biometrics to measure the response to that known event.
...
%TODO conclude lit, relate to project
...
Preliminary testing and experimentation was performed during the work described in \vref{sec:Initial}.
...
Determining the list of times to pass to the null hypothesis algorithm is left as an implementation detail, and is discussed in significant detail later.%TODO NICER
...
\subsubsection{Controlling for Reported Fear}
...
\subsubsection{Controlling for Scare Frequency}
...
\subsubsection{Controlling for `Quality' of Scare Timings}
...
\begin{wraptable}{r}{0.5\linewidth}
	\vspace{-10pt}
    \caption{Relevant statistics for the Regressions}
...
We begin by examining the results of the questions asked to participants after their playtest was completed, before moving on to analyse the raw data from the sensor.
...
A largery study with a bigger sample and tighter control for initial EDA would be better equipped to determine which effects are generalisable.
...
The jump scare was created based on preliminary user testing, and due to the high sensitivity of the sensor, could be made less scary than otherwise necessary, making the game more accessible to players who rarely engage with the horror genre.
