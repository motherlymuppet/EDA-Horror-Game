...
Therefore, this section will begin by examining academic sources in areas related to this topic, before moving to non-academic sources which more closely relate to the study.


\subsection{Fear and the Horror Genre}

\subsubsection{Success of Genre}
Horror as a part of popular culture has no clear beginning, and was even popular in Ancient Greece \citep{jackson81}.
The horror genre is consistently one of the most successful and profitable genres \citep{prince04}.
In one survey conducted in the Netherlands by \citet{johansen13}, 79\% of participants said they enjoyed horror films.
...
\subsubsection{Why Fear is Enjoyable}
...
\subsubsection{Scare Timing}
The timing and pace of scares is important to make them scarier.
There is little research into the ideal timing of jump-scares, but the literature suggests that they should be triggered when the audience `is made to feel that nothing is about to happen' \citep{draven13}.
However, it warns that jump-scares can be `scatter[ed] liberally thoughout the film to the point where it becomes almost numbing' \citep{rosenberg10}.
This reveals that there is a need to pace jump-scares to prevent users from becoming desensitised, or `numb'.


\subsection{Game Design}
\subsubsection{Tailoring Games to Individuals}
...
\subsubsection{Frameworks for Designing Horror Games}
\subsubsection{What People Like in Games}
%replayability
...
\subsubsection{EDA}
%include exampleEda graph

\subsubsection{Wearable Tech}
\subsubsection{HCI}
...
GSR (galvanic skin response) is a synonymous (though outdated) term for EDA, which is notably used in the name of the EDA sensor that the user study uses.
...
\subsubsection{Controlling for Scare Pacing}
