...
As outlined above, the funamental research question is: `\emph{can biometric sensors be used to tailor the timing of scares in a horror game to improve the user experience?}'. To answer this question, the following research objectives must be completed:
...
This project will discuss the previous research and studies that are relevant to the project.
There is little academic research on tailoring horror games to individuals, and none that uses EDA to do so.
Therefore, this section will begin by examining the academic resources most related to this project, before moving to less academic resources which more closely relate to the intended study.
...
The system that was designed and implemented will be discussed in three subsections, each analysing a different part of development.
The \emph{Algorithm} section discusses the system from a purely theoretical perspective, exploring the high-level decisions such as how to adjust scare timings.
...
The \emph{Software Engineering Design} section discusses the processes used when creating the system, and the choices made to ensure that the system was maintainable and easily extensible.
...
The delay cannot begin until this point, which means that the real time between scares is between 10 and 110 seconds.
...
The system is implemented such that it has two interchangeable \emph{storytellers}.
...
Participants were then paired such that both in the pairing gave the same answer.
...
\vspace{-10pt}
	...
	@
	\caption{Initial EDA differs between groups}
...
Analysis of the data and its implications can be found in \vref{sec:Analysis}.

\subsection{User Self-Reported Statistics}
...
All minimum objectives were completed --- a hanted house environment was created for players to explore, and their EDA was tracked while doing so.
