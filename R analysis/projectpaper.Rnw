...
The drop takes place over a period of approximately one second, and the drop size correlates with the degree of alertness.
...
\vref{fig:PerGroupScares} shows the data after being filtered to the 10 seconds after each scare and lines are coloured based on the participant's group.
...
There is a statistically significant positive correlation between the frequency of jump scares and participants' perceived scare frequency (see \vref{fig:ParticipantTimingRegression}).
...
Generalising to the population, players who used the dynamic algorithm did not become desensitised over time ($P = \Sexpr{signif(pValue(interventionRegression), 3)}$).
Comparatively, players in the control group became desensitised over time ($P = \Sexpr{signif(pValue(controlRegression), 3)}$).
