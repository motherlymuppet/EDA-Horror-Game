...
chart = chartDefault +
  ...
  geom_text(aes(x = pointsX, y = pointsY, label = pointsLabel), nudge_y = 6e2, color = "red") +
  ...
  geom_text(aes(x = peakX, y = (peakY + troughY)/2, label = dropText), color = "red", nudge_x = -2e2, family="serif") +
  coord_cartesian(xlim = c(0,11e3))
